...
export function RagSearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.data.results?.length === 0) {
        ...
        showToast(
          `Found ${response.data.results.length} relevant documents`,
          "success"
        );
      }
    } catch (error: any) {
  ...
  const handlePreview = (documentId: string) => {
    window.open(
      `/api/documents/document-preview?documentId=${documentId}`,
      "_blank"
    );
  };
  ...
  const handleDownload = (documentId: string, fileName: string) => {
    window.open(
      `/api/documents/download-document?documentId=${documentId}`,
      "_blank"
    );
